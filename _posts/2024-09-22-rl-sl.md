...
reinforcement learning can be done both offline and online. In framing reinforcement
learning as supervised learning, we can consider that we just collect enough data in
an online fashion from the environment, and train a supervised learning model with the
collected offline data. 
