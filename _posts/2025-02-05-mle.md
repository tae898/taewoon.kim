...
- **Multi-Class Classification (Categorical):**  
  For multi-class classification with one-hot encoded labels, the probability of the correct class is given by the model’s predicted probability, and the negative log-likelihood is:

  $$
  -\sum_{c=1}^K y_c \log \hat{y}_c,
  $$

  which is the **multi-class cross-entropy loss**.
...
Maximum Likelihood Estimation (MLE) is more than just a mathematical formulation—it is a powerful framework that underpins many of the techniques we use in machine learning. By framing parameter estimation as the problem of making the observed data as likely as possible, MLE connects seamlessly to the loss functions we optimize, whether that be Mean Squared Error (MSE) for Gaussian assumptions, Mean Absolute Error (MAE) for Laplace noise, or cross-entropy for classification tasks under Bernoulli/Categorical models.

In essence, choosing a loss function is equivalent to choosing a data distribution: our assumptions about noise and variability directly inform how we penalize errors. This not only affects model accuracy and optimization behavior but also influences how well our model captures the true underlying data-generating process. While MLE yields a single best-fit parameter value—a point estimate—this approach lays the groundwork for more advanced methods that capture uncertainty, which we’ll explore in future posts.

By understanding MLE, you gain a clearer insight into why models are trained the way they are and how to make informed choices about the loss functions that drive your learning algorithms.
